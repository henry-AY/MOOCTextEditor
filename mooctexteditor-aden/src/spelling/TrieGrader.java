...
public class TrieGrader {
    ...
    public static void main(String[] args) {
        ...
        PrintWriter out;
        try {
            out = new PrintWriter("grader_output/TrieGrader.out");
        } catch (Exception e) {
            e.printStackTrace();
            return;
        }
    ...
    private void testPredictions(AutoCompleteDictionaryTrie ac) {
        ...
        int count = 0;
        for (String s : auto) {
        	if ((count == 0 && s.length() == 4) ||
        		(count <5 && s.length() == 6) ||
        		(count >=5 && s.length() == 7)) {
        		feedback.append("  Word "+count+" passed length check.\n");
        	} else {
        		feedback.append("  Word "+count+"("+s+") failed length check.\n");
        	}
        	count++;
        }
        ...
        count = partialList.contains("test") ? ++count:count ;
        count = partialList.contains("tester") ? ++count:count;
        count = partialList.contains("tested") ? ++count:count;
        count = partialList.contains("testin") ? ++count:count;
        count = partialList.contains("teston") ? ++count:count;

        feedback.append("Out of 'test', 'tester', 'tested', 'testin' and 'teston', " + count + " words were found.");
